...
export function generateDataset(schemaInput, options = {}) {
  ...
  for (const table of orderedTables) {
    ...
    for (let index = 0; index < count; index += 1) {
      rows.push(generateRow(table, index, rowsByTable, rules, random, rows, options.rows, schema.byName));
    }
...
function generateRow(table, index, rowsByTable, rules, random, existingRows, rowsSpec, byName) {
  const row = {};
  for (const column of table.columns) {
    ...
    if (column.references) {
      row[column.name] = referencedValue(table, column, index, rowsByTable, random, rowsSpec, byName);
      continue;
    }
...
function referencedValue(table, column, index, rowsByTable, random, rowsSpec, byName) {
  ...
  if (byName && rowsSpec) {
    const parentTable = byName.get(column.references.table);
    if (parentTable) {
      const parentCount = rowCountFor(parentTable, rowsSpec);
      if (parentCount > 0) {
        const parentCol = parentTable.columns.find((c) => c.name === column.references.column);
        const parentIndex = column.unique ? (index % parentCount) : random.int(0, parentCount - 1);

        if (parentCol?.identity || parentCol?.type?.toLowerCase()?.includes("int")) {
          return parentIndex + 1;
        }
        if (parentCol?.type?.toLowerCase()?.includes("uniqueidentifier") || parentCol?.name?.toLowerCase()?.includes("uuid")) {
          const hex = String(parentIndex + 1).padStart(12, "0");
          return `00000000-0000-4000-8000-${hex}`;
        }
        return `value-${parentIndex + 1}`;
      }
    }
  }
